...
void enter_door_player(tile *t) {
    ...
    if(f->cur_room->check) {
        ...
        add_message(f->cur_room->desc);

        if(strcmp(f->cur_room->name, "Ambush") == 0) {
            for(r = 0; r < cvector_size(f->cur_room->entities); ++r)
        ...
        else if(strcmp(f->cur_room->name, "Den of Bears") == 0) {
            f->cur_room->check = false;
...
void attack(entity *from, entity *to){
    switch(from->type){
    case ET_CARROT: case ET_RABBIT: case ET_EGGPLANT: case ET_BEAR: case ET_CAPYBARA: case ET_HORSE: case ET_RUNNER:
    ...
    case ET_LEAF:
        add_message(" The sharp leaves cut through the %s.", to->name);
